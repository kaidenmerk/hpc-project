...
double calcDistance(Point p, Point p2) {
...
int main(int argc, char **argv)
{
    ...
    for(int i = 0; i < epoch; i++) {
        ...
        for(int j = 0; j < k; j++) {
            for(int l = 0; l < ((itemNum / size) + 1); l++) {
                double dist = calcDistance(recvPointData[l], centroidData[j]);
                if(dist < recvPointData[l].minDist) {
                    recvPointData[l].minDist = dist;
                    recvPointData[l].cluster = j;
                }
            }
        }
